...
const SearchByPhoto = () => {
  ...
  const {
    ...
    queryKey: ["trainerDetails", photo], // Adjust queryKey based on your usage
    queryFn: () => fetchTrainerDetails(photo), // Adjust queryFn based on your usage
    // Additional optional properties can be added here if needed
  });

  if (isLoading) return <Spinner />;
  if (error) return <div>Error loading trainer details</div>;

  console.log(trainer); // Check the fetched trainer data in the console
          ...
          <div className="flex flex-col lg:flex-row">
            <div className="lg:w-1/2">
              <img
                src={trainer.photo}
                alt={trainer.name}
                className="h-64 w-64 object-cover rounded-full mx-auto mb-4 lg:mb-0"
              />
            ...
            <div className="lg:w-1/2 lg:pl-8 mt-8 lg:mt-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {trainer.availableSlots.split(",").map((slot, index) => (
                  <Link
                    to={`/book/${trainer.name}/${slot.trim()}`}
                    key={index}
                    className="bg-[#981840] text-white py-2 px-4 rounded-lg hover:bg-[#790f32] transition duration-300 text-center"
                  >
                    {slot.trim()}
                  </Link>
                ))}
